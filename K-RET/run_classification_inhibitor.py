...
def main():
    ...
    parser.add_argument("--weight", default=[0, 1.28],type=str,help="[label0, label1]")

    # Model options.
    parser.add_argument("--batch_size", type=int, default=32, help="Batch size.")
    parser.add_argument("--seq_length", type=int, default=256, help="Sequence length.")
    parser.add_argument("--encoder", choices=["bert", "lstm", "gru", "cnn", "gatedcnn", "attn", "rcnn", "crnn", "gpt", "bilstm"],
    ...
    if torch.cuda.device_count() > 1:
        #print("DEEEEEEEEEEEVIIIIIIIIIIICEEEEEEEEEEEE")
    ...
    def evaluate(args, is_test, metrics='Acc'):
        ...
        if not args.mean_reciprocal_rank:
            ...
            try:
                ...
                print("Acc. (Correct/Total): 0.0000 (0/{}) ".format(len(dataset)))
                if metrics == 'Acc':
                    return 0.0000
                elif metrics == 'f1':
                    return label_1_f1
    ...
    if not args.testing:
        ...
        if torch.cuda.device_count() > 1:
            model.module.load_state_dict(torch.load(args.to_test_model))
        else:
            model.load_state_dict(torch.load(args.to_test_model))
        evaluate(args, True)
